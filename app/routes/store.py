...
@store_bp.route('/api/admin/stores', methods=['GET'])
...
@store_bp.route('/api/admin/stores', methods=['POST'])
def add_store():
    """新增店家資料 (F-04)"""
    try:
        ...
        meal_type = data.get('meal_type', '小吃')
        ...
        try:
            walking_distance = int(walking_distance) if walking_distance is not None else None
        except ValueError:
            return jsonify({"status": "error", "message": "步行時間必須是整數"}), 400
...
@store_bp.route('/api/admin/stores/<int:store_id>', methods=['POST', 'PUT'])
def update_store(store_id):
    ...
    try:
        ...
        if 'name' in data:
            if not data['name']:
                return jsonify({"status": "error", "message": "店名不能為空"}), 400
            update_fields['name'] = data['name']
            
        if 'price_range' in data:
            update_fields['price_range'] = data['price_range']
            
        if 'meal_type' in data:
            update_fields['meal_type'] = data['meal_type']
            
        if 'walking_distance' in data:
            try:
                update_fields['walking_distance'] = int(data['walking_distance']) if data['walking_distance'] is not None else None
            except ValueError:
                return jsonify({"status": "error", "message": "步行時間必須是整數"}), 400
                
        if 'google_maps_url' in data:
            update_fields['google_maps_url'] = data['google_maps_url']
            
        if not update_fields:
            return jsonify({"status": "error", "message": "未提供任何要修改的欄位"}), 400
            
        success = StoreModel.update(store_id, **update_fields)
...
@store_bp.route('/api/admin/stores/<int:store_id>', methods=['DELETE'])
